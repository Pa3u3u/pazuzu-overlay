...
LICENSE="MIT 0BSD Apache-2.0 Apache-2.0 Apache-2.0-with-LLVM-exceptions BSD BSD-2 Boost-1.0 GPL-3 ISC LGPL-3 MIT MPL-2.0 Unicode-DFS-2016 Unlicense ZLIB"
SLOT="0"
KEYWORDS="~amd64"
...
src_configure() {
	local myfeatures=(
		$(usex "dbus" "dbus_mpris dbus_keyring")
	)

	for backend_flag in ${backends}; do
		backend=$(echo ${backend_flag} | tr -d '+-')

		if use ${backend}; then
			myfeatures+=("${backend}_backend")
		fi
	done
...
src_install() {
	...
	mkdir -p "${T}/examples"
	cp "${FILESDIR}/spotifyd-${PV}.conf" "${T}/examples/spotifyd.conf"

	dodoc -r "${T}/examples"

	if use systemd; then
		systemd_douserunit "${S}/contrib/spotifyd.service"
	fi
}

pkg_postinst() {
	elog "The example configuration file has been installed into /usr/share/doc/${PF}."

	if use systemd; then
		elog "The daemon can also be managed by systemd user unit 'spotifyd.service'."
	fi
}
