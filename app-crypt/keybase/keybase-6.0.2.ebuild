...
SRC_URI="
	https://github.com/keybase/client/releases/download/v${PV}/keybase-v${PV}.tar.xz -> ${P}.tar.xz
	verify-sig? (
		https://github.com/keybase/client/releases/download/v${PV}/keybase-v${PV}.tar.xz.sig
			-> ${P}.tar.xz.sig
	)
...
inherit verify-sig systemd go-module
...
src_unpack() {
	...
	default
	ln -vs "client-v${PV}" "${P}" || die
	mkdir -p "${T}/src/github.com/keybase"
	ln -vs "${WORKDIR}/client-v${PV}" "${T}/src/github.com/keybase/client"
}

src_compile() {
	export GOPATH=${T}

	cd "${WORKDIR}/client-v${PV}/go"
